...
export class FollowCamera extends Camera {
  ...
  private readonly cameraLerpFactor: number = 0.12;
  ...
  public override addDistanceInput(delta: number): void {
    this.userDistanceOffset = Math.max(-20, Math.min(80, this.userDistanceOffset + delta));
  }
  ...
  public update(_deltaTime: number): void {
    ...
    this.targetCameraPitch = Cesium.Math.toRadians(-15.0) + this.userPitchOffset;
